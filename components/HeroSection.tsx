  ...
  /** Adds randomized hover wobble without touching inner styles */
  function WobbleOnHover({
    children,
    ampMin = 4,
    ampMax = 10,
    periodMin = 0.9,
    ...
    const { amp, dur } = useMemo(() => {
      const r = Math.random
      return {
        amp: Math.round((ampMin + (ampMax - ampMin) * r()) * 10) / 10,
        dur: Math.round((periodMin + (periodMax - periodMin) * r()) * 100) / 100,
      }
    }, [ampMin, ampMax, periodMin, periodMax])
    ...
    return (
      <motion.div
        ...
        animate={hover ? { y: [0, -amp, 0, amp, 0] } : { y: 0 }}
        transition={hover ? { duration: dur, ease: 'easeInOut', repeat: Infinity } : {}}
  ...
  export default function HeroSection() {
            ...
            >
              ...
              >
                <motion.div variants={stat} className="flex h-[71px] w-[98px] flex-col items-center gap-[12px]">
                  <div className={`${inter.className} w-[77px] text-center text-[32px] font-semibold leading-none text-[#50223C]`}>15K+</div>
                  <div className={`${urbanist.className} w-[98px] text-center text-[13px] font-semibold leading-none text-[#555]`}>Total Downloads</div>
                </motion.div>

                <motion.div variants={stat} className="mt-[3.5px] flex h-[64px] w-[194px] flex-col items-center gap-[9px]">
                  <div className={`${inter.className} w-[60px] text-center text-[32px] font-semibold leading-none text-[#50223C]`}>2K+</div>
                ...
                <motion.div variants={stat} className="mt-[3.5px] flex h-[64px] w-[95px] flex-col items-center gap-[9px]">
                  <div className={`${inter.className} w-[98px] text-center text-[32px] font-semibold leading-none text-[#50223C]`}>4.7 / 5</div>
                  <div className={`${urbanist.className} w-[95px] text-center text-[13px] font-semibold leading-none text-[#555]`}>Review Ratings</div>
                </motion.div>
              </motion.div>
            </motion.div>

            {/* RIGHT (Phone Mockup + Floating Cards) */}
            {/* Responsive scaler wrapper – does not change your inner styles */}
    ...
    >
      <Image
        ...
        fill
        priority
        sizes="(min-width: 1024px) 532px, 80vw"
...
<Reveal once={false} amount={0.1} className="absolute inset-0 z-[5] flex justify-center items-center">
  <PhoneMockupMotion />
                  ...
                  <WobbleOnHover className="absolute top-[80px] left-[600px] lg:left-[70px] z-[300]">
                    <motion.div
                  ...
                  <WobbleOnHover className="absolute top-[60px] right-[-30px] lg:right-[-10px] z-20">
                    <motion.div
                  ...
                  <WobbleOnHover className="absolute bottom-[10px] left-[-60px] lg:left-[25px] z-20">
                    <motion.div
                    ...
                    >
                      ...
                      >
                        <img src="/images/hero-bottom-left.svg" alt="Real-Time Alerts icon" style={{ width: '16px', height: '16px' }} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ color: '#EDE8EB', fontFamily: 'Urbanist', fontSize: '12.509px', fontWeight: 700, lineHeight: '16px', whiteSpace: 'nowrap' }}>
                          Real-Time Alerts
                        </div>
                        <div style={{ color: '#EDE8EB', fontFamily: 'Inter', fontSize: '9.829px', fontWeight: 400, lineHeight: '13px', marginTop: '2px' }}>
                  ...
                  <WobbleOnHover className="absolute bottom-[60px] right-[-80px] lg:right-[-60px] z-20">
                    <motion.div
                      className="flex items-center gap-2 px-3"
                      style={{
                        width: '225.902px',
        ...
        <style jsx>{`
        /* Fix visibility and aspect ratio responsiveness */
.hero-stage > div {
  position: relative;
  z-index: 1;
  min-height: 480px;
...
@media (max-width: 640px) {
  .hero-stage > div {
    min-height: 320px;
          ...
          .hero-root {
            /* Scales the right "stage" so absolute pixels stay visually consistent across 1366→4K */
            --hero-scale: clamp(0.75, min(100vw / 1280, 100vh / 800), 1.6);
          }
          .hero-stage-wrapper {
            width: 100%;
            display: flex;
          ...
          /* Small phones text safety (does not change your original classes; only prevents overflow) */
